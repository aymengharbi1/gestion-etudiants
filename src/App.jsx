...
import { School, Users, BookUser, Book, ClipboardList, Megaphone, LogOut, User, GraduationCap, MessageSquare, Briefcase, ChevronRight, UserCircle, Plus, Edit, Trash2, Search, X, ArrowRightCircle, Save, CalendarDays, BarChart2, ChevronLeft, ChevronRight as ChevronRightIcon, Send, Home, Eye, CheckCircle, XCircle, Clock, BookOpen, Globe, Atom, Calculator, Palette, Settings } from 'lucide-react';

// --- CONFIGURATION FIREBASE ---
// La configuration est maintenant chargée dynamiquement depuis l'environnement d'exécution de la plateforme.
// Cela résout les erreurs en utilisant une configuration valide.
const firebaseConfig = typeof __firebase_config !== 'undefined'
  ? JSON.parse(__firebase_config)
  : {
      apiKey: "VOTRE_API_KEY_PLACEHOLDER",
      authDomain: "VOTRE_AUTH_DOMAIN_PLACEHOLDER",
      projectId: "VOTRE_PROJECT_ID_PLACEHOLDER",
      storageBucket: "VOTRE_STORAGE_BUCKET_PLACEHOLDER",
      messagingSenderId: "VOTRE_MESSAGING_SENDER_ID_PLACEHOLDER",
      appId: "VOTRE_APP_ID_PLACEHOLDER"
    };
    ...
    return (
        <div className="flex h-screen bg-gray-50">
            <aside className="w-64 bg-white shadow-lg flex flex-col transition-all duration-300">
                ...
                <nav className="flex-1 px-4 py-4"><ul>{(menuItems[currentUser?.role] || []).map(item => (<li key={item.id} className="mb-2"><a href="#" onClick={(e) => { e.preventDefault(); setActiveComponent(item.id); }} className={`flex items-center p-3 rounded-lg text-gray-600 hover:bg-blue-50 hover:text-blue-600 transition-colors duration-200 ${activeComponent === item.id ? 'bg-blue-100 text-blue-700 font-bold' : ''}`}><item.icon className="w-5 h-5" /><span className="mr-4">{item.label}</span></a></li>))}</ul></nav>
            ...
            <main className="flex-1 flex flex-col overflow-hidden">
                <header className="h-20 bg-white shadow-sm flex items-center justify-between px-8">
                     <div className="flex items-center"><div className="w-12 h-12 bg-gray-200 rounded-full flex items-center justify-center mr-4"><UserCircle className="w-8 h-8 text-gray-600"/></div>
                        <div><h1 className="text-xl font-bold text-gray-800">مرحبا بك، {currentUser?.name}</h1><p className="text-sm text-gray-500 capitalize">{currentUser?.role === 'admin' ? 'مدير' : currentUser?.role === 'teacher' ? 'معلم' : 'ولي'}</p></div>
                     </div>
                </header>
                <div className="flex-1 overflow-y-auto p-8 bg-gray-100">{renderComponent()}</div>
